...
`references/intake.md` even for detailed one-shot prompts.
...
| **Change journey structure** on an existing journey (add/remove steps, wait days, splits) | Check MCP tool list for a graph-update tool — follow its description and `WithWorkflow`. Topology sketches for **create** only: `references/journey-graph-patterns.md`. If no MCP tool: **Capability boundary** above |
...
| **Journey performance** (opens, clicks, metrics) | Out of scope — Orchestrate read MCP tools (check tool list) |
