...
Duplicate journey names are allowed. Before create, confirm the user wants a **new** journey — not to edit one
that already exists.

- **Clear create intent** ("create", "build", "new journey") — proceed to intake. If they also name a journey
  that already exists, briefly note matches exist and confirm they still want a **new** one (do not block create
  on name alone).
- **Clear edit intent** ("update", "edit", "change my journey") — run `references/identify-journey.md` instead;
  do not call a create tool.
- **Ambiguous** — if they name a journey without create vs edit language, call `listOrchestrateJourneys` and
  ask: edit an existing match, or create a new journey (same name is fine)?
...
After Round 2 (or skips), identify `subId` and `appId` via `listAllApplications` if needed, then create.
