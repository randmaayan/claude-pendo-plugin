...
2. `getOrchestrateJourneySteps` — list Email steps.
3. Each Email step: `messageId` → `emailId` on `updateOrchestrateEmailContent`.
4. Draft HTML; iterate with the user; call the tool per approved email; confirm via tool response or
   Orchestrate UI preview.
...
**Marketing unsubscribe** — required for marketing emails (not transactional):

- If you do not already know `emailType` from create (`templateJson` or built-in template), call
  `getOrchestrateEmail` with the step's `messageId` before drafting.
